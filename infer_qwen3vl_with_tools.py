...
def extract_tool_call(output_text: str) -> dict[str, Any] | None:
    ...
    payload = output_text[start:end] if end != -1 else output_text[start:]
...
def generate_answer_with_tools(
    ...
    """Returns (prediction_text, tool_calls) where each tool call is
    ``{"name": ..., "args": ...}``."""
    messages: list[dict[str, Any]] = [
        {
            "role": "system",
    ...
    with open(args.output_file, "a" if args.resume else "w", encoding="utf-8") as fout:
        for sample in tqdm(dataset, total=len(dataset)):
            ...
            try:
                ...
                prediction, tool_calls = generate_answer_with_tools(
                    model=model,
                    processor=processor,
                    image=image,
                    question=question,
